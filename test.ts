...
test("Calcuting the tax on a number", (): void => {
  expect((): number => Number(1).tax(101)).toThrow();
  expect((): number => Number(1).tax(-100)).toThrow();
...
test("Calculating the tax on a number with the tax applied to the number", (): void => {
  expect((): number => Number(1).withTax(101)).toThrow();
  expect((): number => Number(1).withTax(-1)).toThrow();
...
test("Calculating the monthly payments for a mortage", (): void => {
  expect((): number => Number(100000).mortage(-1, 1)).toThrow();
  expect((): number => Number(100000).mortage(0, 0)).toThrow();
  expect((): number => Number(100000).mortage(0, -1)).toThrow();
